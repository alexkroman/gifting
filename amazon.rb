...
get '/give' do
  ...
  @item_list = Item.all(:asin => asins.first, :asin.not => session[:seen]).sort{|a,b| a.surveys.all(:like => true).size - a.surveys.all(:like => false).size <=> b.surveys.all(:like => true).size - b.surveys.all(:like => false).size}.reverse
  @item_list = @item_list + Item.all(:asin.not => asins, :asin.not => session[:seen]).sort{|a,b| a.surveys.all(:like => true).size - a.surveys.all(:like => false).size <=> b.surveys.all(:like => true).size - b.surveys.all(:like => false).size}.reverse
